...
import ShareModal from '../../components/drive/ShareModal/ShareModal';
...
export default function MyDrivePage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load drive items from server.');
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteItem = async (itemToDelete) => {
    try {
      ...
      const typeLabel = itemToDelete.itemType === 2 ? 'Folder' : 'File';
  ...
  const handleFolderTitleClick = () => {
    if (currentParentId) {
      ...
      navigate(`/drive/folder/${currentParentId}`, {
        state: {
  ...
  return (
    <Box sx={{ height: '100%' }}>
      ...
      {/* Item Actions Right-Click / Button Menu */}
